...
export function PurchaseBurst({ item, onDone }: PurchaseBurstProps) {
  useEffect(() => {
    const timer = window.setTimeout(onDone, 1500);
    return () => window.clearTimeout(timer);
  }, [onDone]);

  const gained = item.drops !== undefined;
    ...
    <motion.div
      ...
      <motion.div
        ...
        <motion.img
          src={item.icon}
          alt=""
          className="h-24 w-24 rounded-xl object-cover ring-2 ring-gold"
          animate={{ rotate: [0, -6, 6, 0] }}
          transition={{ duration: 0.6, ease: 'easeInOut' }} />
        
        <p className="font-serif text-lg font-semibold text-gold-soft">{item.name}</p>
        <motion.p
          ...
          {gained ? `+${item.drops?.toLocaleString()}` : item.contents}
          {gained && <BloodDrop className="h-5 w-5 text-blood" />}
        </motion.p>
      </motion.div>
