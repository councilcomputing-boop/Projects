      ...
      <div className="flex max-h-[80vh] max-w-xs flex-col items-center gap-4">
          ...
          <motion.div
            ...
            <motion.div
              ...
              transition={{ duration: 0.9, delay: Math.min(i, 12) * 0.05 + 0.25 }}>
                    <CardBackSprite back={result.back} width={70} />
                  </motion.div> :
            result.alreadyOwned ?
            <CardBackSprite back={result.back} width={70} className="opacity-55" /> :
            ...
            <div className="relative aspect-[5/7] overflow-hidden rounded-xl bg-ink shadow-card" style={{ width: 70 }}>
                ...
                <p className={`text-center text-[9px] ${result.unlocked ? 'font-bold text-gold' : 'text-gold-soft'}`}>{resultLabel(result, chest)}</p>
              </motion.div>
          )}
          </div>
          <button
